...
import { ReactLogoModel } from "./react-logo-model";
  ...
  useFrame((state) => {
    if (ref.current) {
      ...
      ref.current.rotation.x =
        Math.cos(state.clock.getElapsedTime() * 0.3) * 0.12;
    }
  });
...
export function ReactLogoScene() {
  return (
    <CanvasErrorBoundary
      fallback={
        <div className="flex h-full w-full items-center justify-center bg-neutral-900 text-white">
          <p className="text-sm text-neutral-400">React 3D Model Preview</p>
        </div>
      }
    >
      <Canvas
      ...
      >
        ...
        {/* Key Light casting specular highlights */}
        <directionalLight
          position={[5, 6, 6]}
          intensity={2.2}
          castShadow
        ...
        {/* Vibrant React Cyan Accent Point Light */}
        <pointLight
          position={[-4, -2, 4]}
          intensity={1.8}
          color="#61dafb"
