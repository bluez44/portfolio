...
/**
 * Three.js 3D procedural reconstruction of the Java logo (public/java.svg).
 * Preserves exact paths, colors, and proportions from the original SVG.
 */

const JAVA_SVG_STRING = `<?xml version="1.0" encoding="utf-8"?><!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
<svg width="800px" height="800px" viewBox="-118.513 4.399 540.906 540.906" xmlns="http://www.w3.org/2000/svg">
...
export function JavaLogoModel({ scale = 1 }: { scale?: number }) {
  ...
  useFrame((state) => {
    if (groupRef.current) {
      ...
      groupRef.current.position.y = Math.sin(t * 1.5) * 0.2; // A bit larger float
    }
  });
          ...
          return (
            ...
            >
              <meshPhysicalMaterial
                color={mesh.color}
                roughness={isOrange ? 0.3 : 0.15}
                metalness={isOrange ? 0.1 : 0.25}
                emissive={mesh.color}
                emissiveIntensity={isOrange ? 0.15 : 0.05}
                clearcoat={1.0}
