...
const REACT_CYAN_EMISSIVE = "#1b4d5e";

// Custom 3D Ellipse Curve for TubeGeometry
...
const TEXT_EXTRUDE_SETTINGS: THREE.ExtrudeGeometryOptions = {
  depth: 14,
  bevelEnabled: true,
  bevelThickness: 1.5,
  bevelSize: 1.5,
...
export function ReactNativeLogoModel({ scale = 1 }: { scale?: number }) {
  ...
  const tubeGeometry = useMemo(() => {
    const curve = new EllipseCurve3D(2.0, 0.75);
    const geo = new THREE.TubeGeometry(curve, 64, 0.07, 16, true);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const svgData = loader.parse(RN_SVG_STRING);
      ...
      if (shapes.length > 0) {
        const geo = new THREE.ExtrudeGeometry(shapes, TEXT_EXTRUDE_SETTINGS);
        geo.computeVertexNormals();

        // Translate to center of 2224x2666 SVG viewbox area
        geo.translate(-1782, -1350, 0);

        // Scale: X is positive (+0.0016), Y is negative (-0.0016) to orient Y-down SVG upright,
        // Z is positive (+0.0016) so extrusion points forward out towards the camera
        geo.scale(0.0016, -0.0016, 0.0016);
        setTextGeometry(geo);
      }
    } catch (err) {
      console.warn("SVGLoader parsing error for React Native logo:", err);
    }
  }, []);

  return (
    <group ref={groupRef} scale={scale}>
      {/* Central React Atom Nucleus Sphere */}
      ...
      {/* 3D Gyroscopic Elliptical Orbital Rings */}
